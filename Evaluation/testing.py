...
def test(model_path,
    ...
    with open(model_path, "rb") as f:
        ...
        # #XXX print FK tables
        # bound_ensemble_nc.schema.get_FK_tables()
        # exit()
    ...
    for i, q_name in enumerate(all_queries):
        ...
        if os.path.exists(query_predicate_location + '/' + q_name + '.pkl'):
            #XXX for FactorJoin + filter-during-sample
        ...
        if True:
            ...
            ests, c_time, m_time, s_time, hists = bound_ensemble_nc.get_cardinality_bound_all(
                all_queries[q_name],
            ...
            if no_multi_PE:
                # write new results
                output = copy.deepcopy(bound_ensemble_nc.query_predicate)
                output['results'] = hists
                #XXX for debugging, write just one
                #if os.path.exists(output_predicate_location + '/' + q_name + '.pkl'):
                with open(output_predicate_location + '/' + q_name + '.pkl', 'wb') as f:
                    pickle.dump(output, f)
                continue

            # c_time = initialize model + query conversion time
            # m_time = initialize model time
            # s_time = initialize sample time
            # q_time = initialize model + initialize sample + else - query conversion
